...
df[["gyr_y", "label"]].plot.box(by="label", figsize=(20, 10))
df[outlier_columns[:3] + ["label"]].plot.box(
...
for col in outlier_columns: 
    for label in df["label"].unique():
        dataset = mark_outliers_chauvenet(df[df["label"] == label], col)
        #Replacing values marked in outliers into NAN 
        dataset.loc[dataset[col + "_outlier"], col] = np.nan
        #Upate coloum in the orginal Data Frame
